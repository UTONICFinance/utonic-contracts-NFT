...
export async function run() {
  ...
  const fs = require('fs');
  //fs.writeFileSync("wallet.pk", key.secretKey);
  //fs.writeFileSync("wallet.addr", key.publicKey);
  const publicKey = fs.readFileSync('./wallet.addr');
  const secretKey = fs.readFileSync('./wallet.pk');
